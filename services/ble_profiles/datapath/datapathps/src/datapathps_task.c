...
#ifndef custom_datapath //by pang
/*
...
{	
    ...
    {
        // TX ccc
        if (param->handle == (datapathps_env->shdl + DATAPATHPS_IDX_NTF_CFG))
        {
			uint16_t value = 0x0000;
        ...
        // RX data
        else if (param->handle == (datapathps_env->shdl + DATAPATHPS_IDX_VAL))
        {
			//inform APP of data
            struct ble_datapath_rx_data_ind_t * ind = KE_MSG_ALLOC_DYN(DATAPATHPS_RX_DATA_RECEIVED,
                    prf_dst_task_get(&datapathps_env->prf_env, conidx),
...
{	
    ...
    if (param->handle == (datapathps_env->shdl + DATAPATHPS_IDX_DESC)) {
        cfm->length = sizeof(custom_tx_desc);
        memcpy(cfm->value, custom_tx_desc, cfm->length);
    }else if (param->handle == (datapathps_env->shdl + DATAPATHPS_IDX_NTF_CFG)) {
        uint16_t notify_ccc;
        if (datapathps_env->isNotificationEnabled[conidx]) {
            notify_ccc = 1;
        } else {
...
static void send_notifiction(uint8_t conidx, const uint8_t* ptrData, uint32_t length)
{
    ...
    if (datapathps_env->isNotificationEnabled[conidx]) {
        ...
        // Fill in the parameter structure
        report_ntf->operation = GATTC_NOTIFY;
        report_ntf->handle    = datapathps_env->shdl + DATAPATHPS_IDX_VAL;
        // pack measured value in database
        report_ntf->length    = length;
        memcpy(report_ntf->value, ptrData, length);
        // send notification to peer device
                                      ...
                                      ke_task_id_t const src_id)
{
	...
	prf_gatt_write(&(datapathps_env->prf_env), KE_IDX_GET(dest_id), datapathps_env->shdl + DATAPATHPS_IDX_VAL,
                                   (uint8_t *)&param->value, param->length, GATTC_WRITE_NO_RESPONSE);
                                        ...
                                        ke_task_id_t const src_id)
{
    ...
    switch(param->type)
    {
        ...
        case GATTC_NOTIFY:
        {
            if (param->handle == datapathps_env->shdl + DATAPATHPS_IDX_VAL)
            {
				//inform APP of data
				struct ble_datapath_rx_data_ind_t * ind = KE_MSG_ALLOC_DYN(DATAPATHPS_NOTIFICATION_RECEIVED,
						prf_dst_task_get(&datapathps_env->prf_env, conidx),
                                      ...
                                      ke_task_id_t const src_id)
{
	...
	prf_gatt_write(&(datapathps_env->prf_env), param->connecionIndex, datapathps_env->shdl + DATAPATHPS_IDX_NTF_CFG,
                                   (uint8_t *)&ccc_write_val, sizeof(ccc_write_val), GATTC_WRITE);
